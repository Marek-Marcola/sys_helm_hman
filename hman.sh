...
if [ $HELP -eq 1 ]; then
  ...
  echo "$(basename $0) -e [args2]       # exec"
  echo "$(basename $0) -rl              # repo list"
  echo "$(basename $0) -ru [repo]       # repo update"
  echo "$(basename $0) -r [keyword]     # repo search"
  echo "$(basename $0) -c               # chart info"
  echo "$(basename $0) -cr              # chart readme"
  echo "$(basename $0) -cv              # chart values"
  echo "$(basename $0) -D               # app diff yaml"
  echo "$(basename $0) -init [-x]       # app init show/run"
  echo "$(basename $0) -iu   [-x]       # app install/update"
  echo "$(basename $0) -u    [-x]       # app uninstall"
  echo "$(basename $0) -a               # app list"
  echo "$(basename $0) -ah              # app history"
  echo "$(basename $0) -am              # app manifest"
  echo "$(basename $0) -ac              # app chart"
  echo "$(basename $0) -al              # app log"
  echo "$(basename $0) -RR              # res rollout restart"
  echo "$(basename $0) -RH              # res rollout history"
  echo "$(basename $0) -RS              # res rollout status"
  echo "$(basename $0) -Rn              # res scale replicas to n"
  echo "$(basename $0) -la              # spooler load,archive"
  echo "$(basename $0) -ls              # spooler list"
  ...
  echo "$(basename $0)                  # env list/app history"
  echo ""
  echo "opts:"
  echo "  -A  release name"
  ...
  echo "notes:"
  echo "  hm -L -x         # link"
  echo "  ap-name -init -x # init"
  echo ""
  echo "  ap-name -ru -r   # repo update/show"
  echo "  ap-name -E       # edit"
  echo "  ap-name -D       # diff"
  echo "  ap-name -iu -x   # install/update"
  echo "  ap-name -u -x    # uninstall"
  exit 0
fi
